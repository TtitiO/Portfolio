...
export const BlogCard: React.FC<BlogCardProps> = ({ blog, view = 'list' }) => {
    const parsedDescription = { __html: marked.parse(blog.description) };

    if (view === 'grid') {
        return (
            <a href={blog.link} target="_blank" rel="noreferrer noopener" className="block group h-full">
                <div className="h-full flex flex-col rounded-md transition p-4 motion-reduce:transition-none border border-border dark:border-dark-surface bg-surface dark:bg-dark-surface lg:hover:shadow-lg lg:hover:border-accent dark:lg:hover:border-dark-accent">
                    <header className="mb-3 text-xs font-semibold uppercase tracking-wide text-secondary dark:text-dark-secondary">
                        {new Date(blog.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                    </header>
                    <div className="flex-grow">
                        <h3 className="font-medium leading-snug text-heading dark:text-dark-heading group-hover:text-accent dark:group-hover:text-dark-accent">{blog.title}</h3>
                        <div
                            className="mt-2 text-sm leading-normal markdown-content text-secondary dark:text-dark-secondary line-clamp-3"
                            dangerouslySetInnerHTML={parsedDescription}
                        />
                    </div>
                    <footer className="mt-4">
                        <ul className="flex flex-wrap" aria-label="Tags:">
                            {blog.tags.map((tag, i) => (
                                <li key={i} className="mr-1.5 mt-2">
                                    <div className="flex items-center rounded-full bg-green/10 px-3 py-1 text-xs font-medium leading-5 text-green dark:text-dark-green ">{tag}</div>
                                </li>
